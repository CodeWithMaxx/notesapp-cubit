...
class _MainPageState extends State<MainPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("𝑵𝑶𝑻𝑬𝑩𝑶𝑶𝑲",style: TextStyle(fontSize: 20),),
        centerTitle: true,
      ...
      body: BlocBuilder<NotesCubit,NoteState>(
        builder: (context,state){
          if(state is loadingState){
            ...
            return ListView.builder(itemCount:state.noteList.length,itemBuilder: (context,index){
              return Padding(
                ...
                child: Container(
                  ...
                  child: ListTile(
                    onTap: (){
                    Navigator.push(context, MaterialPageRoute(builder: (context)=>NotesView(state.noteList[index].id, state.noteList[index].title, state.noteList[index].desc))).then((result){
                        if (result != null) {
                          ...
                          context.read<NotesCubit>().addnotes(NotesModel(title: updatedTitle, desc: updatedDesc));
                        }
                    ...
                    title: Text("${state.noteList[index].title}",style: TextStyle(fontWeight: FontWeight.bold,fontSize: 18),),
                    ...
                    trailing: IconButton(onPressed: (){
                      showAdaptiveDialog(context: context, builder: (BuildContext context){
                        return AlertDialog.adaptive(
                          ...
                          content: Text("Are you sure to delete this note please confirm"),
                          actions: [
                            Row(mainAxisAlignment: MainAxisAlignment.end,
                              children: [
                                IconButton(onPressed: (){
                                  Navigator.of(context).pop();
                                }, icon: const Text("No",style: TextStyle(fontWeight: FontWeight.bold),)),
                                SizedBox(width: 100,),
                                IconButton(onPressed: (){
                                  BlocProvider.of<NotesCubit>(context).deletenotes(state.noteList[index].id!);
                                  ScaffoldMessenger.of(context).showSnackBar(
                                   const SnackBar(content: Text("Note Deleted"))
                                  );
                                  Navigator.of(context).pop();
                                }, icon: Text("Yes",style: TextStyle(fontWeight: FontWeight.bold),)),
                              ],
                            )
                          ],
                        );
                      });
                    }, icon: const Icon(Icons.delete_forever_outlined,size: 27,)),
                  ),
                ),
              );
            });
            ...
            showDialog(context: context, builder: (BuildContext context){
              return AlertDialog(
                title: Text("Warning",style: TextStyle(fontSize: 17,fontWeight: FontWeight.bold,color: Colors.red),),
